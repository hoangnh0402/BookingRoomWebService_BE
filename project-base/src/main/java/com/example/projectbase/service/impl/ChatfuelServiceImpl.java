...
import com.example.projectbase.service.ChatBotService;
import lombok.RequiredArgsConstructor;
import org.apache.commons.lang3.ObjectUtils;
import org.springframework.beans.factory.annotation.Value;
import com.example.projectbase.domain.dto.chatfuel.*;
...
public class ChatfuelServiceImpl implements ChatBotService {
    ...
    private StringBuffer formatTextRoomResponse(Room room) {
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("✅ Là loại phòng ").append(room.getType()).append("\n");
        stringBuffer.append("✅ Phòng có diện tích ").append(room.getSize()).append("m2. ")
                .append("Với ").append(room.getBed()).append(".\n");
        stringBuffer.append("✅ Phòng có các dịch vụ sẵn có như: ").append(room.getServices()).append("\n");
        stringBuffer.append("✅ ").append(room.getDescription());
        return stringBuffer;
    }
    ...
    private List<TextResponse> setTextResponseSale(Sale sale) {
        List<TextResponse> textResponses = new LinkedList<>();
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("Phòng đang được giảm giá ").append(sale.getSalePercent()).append("%.\n");
        stringBuffer.append("Thời gian kết thúc giảm giá: ").append(formatter.format(sale.getDayEnd())).append(".");
        textResponses.add(new TextResponse(stringBuffer.toString()));
        return textResponses;
    }
    ...
    private List<TextResponse> setTextResponseService(Service service) {
        List<TextResponse> textResponses = new LinkedList<>();
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append("\uD83C\uDF8A ").append(service.getTitle()).append("\n");
        stringBuffer.append("\uD83D\uDCA5 Giá ").append(currencyVN.format(service.getPrice())).append("/người\n");
        stringBuffer.append("\uD83C\uDF24 ").append(service.getDescription()).append("\n");
        textResponses.add(new TextResponse(stringBuffer.toString()));
        return textResponses;
    }
    ...
    private List<TextResponse> setTextResponseProduct(Product product) {
        List<TextResponse> textResponses = new LinkedList<>();
        StringBuffer stringBuffer = new StringBuffer();
        stringBuffer.append(product.getName()).append("\n");
